...
async def _process(token: str) -> tuple[bool, str]:
    ...
    status, body = await send_unsubscribe_to_crm(
        email, entity_id=entity_id, thread_id=thread_id, token=auth_token,
    )
    duration_ms = int((time.perf_counter() - start) * 1000)

    if status and 200 <= status < 300:
        logger.info(
            "unsubscribe processed email=%s entity_id=%s thread_id=%s status=%s duration_ms=%d",
            email, entity_id, thread_id, status, duration_ms,
        )
        return True, email

    logger.error(
        "unsubscribe crm_forward_failed email=%s entity_id=%s thread_id=%s status=%s duration_ms=%d body=%s",
        email, entity_id, thread_id, status, duration_ms, (body or "")[:500],
    )
